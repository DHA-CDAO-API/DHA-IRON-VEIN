...
import { seedCasualtyReferenceData } from "./casualty-reference";
import {
  computeDailyDemand,
...
export async function runSeed(opts: SeedOptions = {}): Promise<void> {
  ...
  // The four casualty reference tables (patient_types,
  // patient_item_requirements, event_types, event_patient_mix) are seeded
  // through a focused helper so the same data is available to the
  // server-boot self-heal path (see lib/casualty-reference). The helper
  // is idempotent — per-table empty-count checks let it act as both the
  // initial fill (after a TRUNCATE) and the self-heal path.
  await seedCasualtyReferenceData();
