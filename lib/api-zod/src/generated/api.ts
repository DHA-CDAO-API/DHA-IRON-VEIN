...
export const ListSitesResponseItem = zod.object({
  ...
  type: zod.string(),
  regionalHub: zod.string().nullish(),
  aor: zod.string().nullish(),
  coordsApproximate: zod.boolean().optional(),
  hiddenFromMap: zod.boolean().optional(),
  daysOfSupply: zod.number(),
  openAlerts: zod.number(),
  riskScore: zod.number(),
  criticalShortItems: zod.number().optional(),
...
export const GetCurrentAuthUserResponse = zod.object({
  ...
  mfa: zod.object({
    enrolled: zod.boolean().describe("User has completed TOTP enrollment."),
    verified: zod
      .boolean()
      .describe("Current session has passed MFA challenge."),
    required: zod
      .boolean()
      .describe("MFA is required for this user (always true here)."),
    sessionExpiresAt: zod.coerce.date().nullish(),
  }),
});
...
export const GetMfaStatusResponse = zod.object({
  enrolled: zod.boolean().describe("User has completed TOTP enrollment."),
  verified: zod.boolean().describe("Current session has passed MFA challenge."),
  required: zod
    .boolean()
    .describe("MFA is required for this user (always true here)."),
  sessionExpiresAt: zod.coerce.date().nullish(),
 ...
 */
export const StartMfaEnrollmentResponse = zod.object({
  secret: zod
    .string()
    .describe("Base32-encoded TOTP secret to manually key into the app."),
  otpauthUri: zod.string().describe("otpauth:\/\/ URI for QR encoding."),
  qrSvg: zod
    .string()
    .describe("Inline SVG of the QR code for the otpauth URI."),
  issuer: zod.string(),
  account: zod.string(),
});
 ...
 */
export const verifyMfaEnrollmentBodyCodeMin = 6;
export const verifyMfaEnrollmentBodyCodeMax = 16;

export const VerifyMfaEnrollmentBody = zod.object({
  code: zod
    .string()
    .min(verifyMfaEnrollmentBodyCodeMin)
    .max(verifyMfaEnrollmentBodyCodeMax)
    .describe(
      "6-digit TOTP code from authenticator, or single-use recovery code.",
    ),
});

export const verifyMfaEnrollmentResponseRecoveryCodesMin = 8;
export const verifyMfaEnrollmentResponseRecoveryCodesMax = 16;

export const VerifyMfaEnrollmentResponse = zod.object({
  enrolled: zod.boolean(),
  recoveryCodes: zod
    .array(zod.string())
    .min(verifyMfaEnrollmentResponseRecoveryCodesMin)
    .max(verifyMfaEnrollmentResponseRecoveryCodesMax)
    .describe("One-time recovery codes — shown ONCE."),
});
...
export const verifyMfaBodyCodeMax = 16;

export const VerifyMfaBody = zod.object({
  code: zod
    .string()
    .min(verifyMfaBodyCodeMin)
    .max(verifyMfaBodyCodeMax)
    .describe(
      "6-digit TOTP code from authenticator, or single-use recovery code.",
    ),
});
